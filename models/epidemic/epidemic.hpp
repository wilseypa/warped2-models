...
WARPED_DEFINE_OBJECT_STATE_STRUCT(EpidemicState) {
...
class EpidemicObject : public warped::SimulationObject {
public:

    EpidemicObject(const std::string& name, 
...
private:
    ...
    void refreshLocationState(unsigned int current_time);

    void updateIntraLocDiseaseSpread(
                            std::map<unsigned int, std::shared_ptr<Person>> population, 
                            unsigned int current_time);

    void triggerDiffusion(unsigned int current_time);

    void migratePerson(unsigned int current_time);
